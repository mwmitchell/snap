module Snap::Zone::Callbacks
  ...
  def before(options=nil, &block)
    before_blocks << Snap::Zone::Event::Base.new(self, options, &block)
  end

  def after(options=nil, &block)
    after_blocks << Snap::Zone::Event::Base.new(self, options, &block)
  end
  
  #
  #
  #
  def execute_before_filters(action=nil)
    action||=@action
    result = []
    result += parent.execute_before_filters(action) if parent
